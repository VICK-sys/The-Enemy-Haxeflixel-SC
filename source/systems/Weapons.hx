...
class Weapons
{
	static var WEAPON_MODES:Array<Array<WeaponMode>> = [[Swing, Slice, Throw], [Hammer, Quake], [Bow, Rain], [Hook]];
	...
	public function new(player:Player, scythe:FlxSprite, arena:Arena, director:EnemyDirector, status:PlayerCombat, fx:Fx, pickups:Pickups)
	{
		this.player = player;
		this.status = status;
		hits = new HitPipeline(status, fx, pickups);
	...
	{
		if (status.dead || superScythes.activating)
			return;

		if (FlxG.mouse.justPressedRight && weapon == 0 && !superScythes.active() && !throwAttack.airborne && !hookAttack.busy && status.canSuper())
		{
			status.spendSuper();
			superScythes.activate();
			return;
		}
