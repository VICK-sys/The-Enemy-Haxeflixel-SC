...
`RETIRED_KEYS` lists save fields that belonged to features the game no longer has. Binding the save deletes any of them that are present and writes the file back once. A save keeps every key ever written to it, so a player who used a removed feature carries its value forever, and a later feature that picks the same name inherits a stale value instead of its own default. Retiring the name is what stops that. The chat window position went first: the window was draggable once, and every save from that build still held the spot it was left in.
