...
A shutter covers the counter while the shop is shut and rolls up when it opens. Behind it, a dark interior backdrop fills the opening, pinned to the ground layer so the shutter always slides over it. Both draw behind the building, and the counter opening is the only transparent part of the art, so the opening is the only place either can show. Sliding the shutter up tucks it behind the sign board, which clips it without any clipping code. It rolls back down when the shop closes. The building itself keeps its full colour whether open or shut; the shutter position is the signal, not a tint.
