...
While the shine holds, the hammer sparkles on a beat: the `charge_glow` pop from the crossbow's full draw plays at the hammer's head every `SPARK_GAP`, alongside a white flash, starting on the ready chime itself. A release inside `SPARK_WINDOW` of a sparkle is a timed release and multiplies the giga's damage by the block's `sparkMult`, flashes the hammer as it goes, and travels the wire as the perfect flag so remote players see sparks on the slash. A timed swing that connects lands harder in the mix too: the impact sound plays at full volume and pitched down, with the heavy hammer thunk layered under it, where an untimed hit plays it plainly. The first sparkle rides the chime, so releasing right as the shine lands is itself a timed hit.
