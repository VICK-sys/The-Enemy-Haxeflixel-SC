...
The rapid sweeps use one arc config and the finisher another, both the same shape as a normal swing block. A sweep reaches exactly as far and as wide as a normal hammer swing, since it copies that block's `spawnDist`, `meleeRange` and `meleeArcCos`. It differs only in what suits a string: a single point of damage, a light hitstop so the swings flow, and almost no knockback, which keeps a target in reach for the swings still to come. The finisher reaches further still, for four damage with the heavy stop and the giga swing sound. Both deflect enemy fire, so the string clears the air in front while it grinds.
