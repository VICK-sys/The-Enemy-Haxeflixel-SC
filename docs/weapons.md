...
The timing reward sits on the shine itself. The `charge_glow` pop from the crossbow's full draw plays once at the hammer's head as the shine lands, and a release inside `SHINE_WINDOW` of that moment is a timed release: it multiplies the giga's damage by the block's `shineMult`, flashes the hammer as it goes, and travels the wire as the perfect flag so remote players see sparks on the slash. A timed swing that connects lands harder in the mix too: the impact sound plays at full volume and pitched down, with the heavy hammer thunk layered under it, where an untimed hit plays it plainly. There is one window per charge. Holding past it keeps the giga ready but plain, so the choice is between releasing on the beat for the bonus and holding for the moment that suits the fight.
...
The rapid sweeps use one arc config and the finisher another, both the same shape as a normal swing block, and both now carry the giga swing's hit: its `spawnDist`, `meleeRange`, `meleeArcCos`, damage and effect scale. Every blow in the string is a giga hit. What the two blocks keep of their own is pacing, not power. The sweeps take a light hitstop so the string flows and almost no knockback, which holds a target in reach for the blows still to come. The finisher keeps the heavy stop, the real knockback and the giga swing sound, and it alone carries `shineMult`.

The finisher fires boosted, which is the same flag a giga swing released on its shine sets. It multiplies the hit by `shineMult`, travels the wire as the perfect flag so remote players see sparks on that slash, and picks the boosted branch of the giga hit sound: the impact at full volume pitched down, with the hammer thunk layered under it, where the sweeps play it plainly. One `gigaHit` function builds that sound for the charged giga swing and for both halves of the flurry, so there is a single definition of what a giga hit sounds like. Both deflect enemy fire, so the string clears the air in front while it grinds.
